    ...
    if (!turn) {
        if (bridgeState.activeThreadId && bridgeState.activeTurnId) {
            try {
                ...
                await telegram.sendMessage(chatId, `Stop failed: ${clip(oneLine(String(error.message ?? error)), 500)}`);
            }
            return;
        }
...
async function handleCallback(update) {
    ...
    if (steerMatch) {
        ...
        if (action === "c") {
            ...
            try {
                ...
                await telegram.sendMessage(pending.chatId, `Stop failed: ${clip(oneLine(String(error.message ?? error)), 500)}`);
            }
            return;
        }
    ...
    if (cancelMatch) {
        ...
        try {
            ...
            turn.cancelRequested = false;
            await updateTurnMessage(turn, true);
            await telegram.sendMessage(turn.chatId, `Stop failed: ${clip(oneLine(String(error.message ?? error)), 500)}`);
        }
        return;
    }
